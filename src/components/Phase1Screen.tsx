...
const Phase1Screen: React.FC<Phase1ScreenProps> = ({
  ...
  // Initialize the service when the game starts
  useEffect(() => {
    initBuzzerService();
  }, []);

  // Update game state whenever phase2 questions or current question changes
  useEffect(() => {
  ...
  // In your main Game component
  useEffect(() => {
    const handleTeamActivation = (event: CustomEvent) => {
      if (event.detail && typeof event.detail.teamIndex === "number") {
        ...
        // Store active team info for remote devices to check
        localStorage.setItem("active_team", event.detail.teamIndex.toString());
      }
    };

    document.addEventListener(
      "activateTeam",
      handleTeamActivation as EventListener
    );
      ...
      <div className="max-w-4xl">
        ...
        >
          ...
          >
            ...
            <p>
              Each team has{" "}
              <span className="font-bold text-algerian-green">30 seconds</span>{" "}
      ...
      <div className="w-full max-w-4xl">
        ...
        >
          ...
          <p className="text-xl mb-8">
            Get ready for Phase 2: The Speed Round!
          </p>
  ...
  return (
    <div className="w-full space-y-6 max-w-5xl mx-auto p-6">
      {/* Phase indicator */}
            ...
            <motion.div
              ...
              className={`
                ...
                ${
                  gameState.answerLocked
                    ? "cursor-default"
                    : "hover:border-primary-light hover:shadow hover:-translate-y-1 transition-transform"
                }
              `}
      ...
      <div className="text-center text-sm text-slate-500">
        ...
        {gameState.phase1Questions.length}
      </div>
    </div>
  );
};
